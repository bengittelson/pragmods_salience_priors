...
for (i=0; i<expt[0].length; i++) {
    base_image_pl[i] = new Image();
    base_image_pl[i].src = "images" + file_number_to_use_for_referents + "/" + base + "-base" + String(i+1) + ".png";
}
...
//NOTE: To return this to how it originally was, delete the else block and just use the code inside of the if block.
for (i=0;i<props.length;i++) {
    props_image_pl[i] = new Image();
    props_image_pl[i].src = "images" + file_number_to_use_for_referents + "/" + base + "-" + props[i] + ".png";
}
...
var experiment = {
	...
	distractor_property: distractor_props_list, 
	...
	distractor_position: distractor_position_array, 
	...
	next_test: function() {
		...
		if (participant_response_type == 0) {
			...
			for (i = 0; i < expt.length; i++) {
				...
				' class="notChoices objTable" ' +
				//Should this be String(i+expt[0].length?)
				//'id=\"tdchoice' + String(i+3) + '\">';
				//'id=\"tdchoice' + String(i+expt[0].length) + '\">';
				'id=\"tdchoice' + String(i+expt.length) + '\">';
		...
		if (question_type == 0) { // LISTENER CONDITION
			...
			} else if (linguistic_framing == 11) {
				label_html += '<p class="block-text style="font-size:x-large;">' + 'Bob can only say one word to commmunicate with you which Christmas tree he will trim next, and he says: <b>mumblemumble</b>.</p>'; 
				label_html += '<p class="block-text style="font-size:x-large;">' + "(You couldn't hear what he said.)</p>"; 
				//Are the double quotes going to cause a problem? 
				label_html += '<p class="block-text style="font-size:x-large;">' + 'Click below on the option that represents the <b>' + base + '</b> that you think Bob is talking about.</p>'; 
			}
